...
/**
 * @file gl_camera.hpp
 * Provides a wrapper for an equivalent to the gluLookAt command
 *
 * @brief Keeps track of orientation, zoom, and view point to compute
 * the correct camera view. This provides more intuitive commands
...
class GLCamera
{
 ...
 public:
  ...
  void set_perspective(double fovy, double aspect,
    ...
    GLdouble ymax = znear * std::tan(fovy * M_PI / 360.0);
    GLdouble ymin = -ymax;
   ...
   */
  void view_point(float x, float y, float z) {
    point = Point(x, y, z);
  }
  /** @overload */
  void view_point(const Point& p) {
    point = p;
  }
  ...
  void rotate_x(float angle) {
    ...
    eyeV = eyeV * std::cos(angle) + upV * std::sin(angle);
  ...
  void rotate_y(float angle) {
    ...
    eyeV = eyeV * std::cos(angle) + rightV * std::sin(angle);
    // Normalize for numerical stability
    eyeV /= norm(eyeV);
